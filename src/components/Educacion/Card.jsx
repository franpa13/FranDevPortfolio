...
export default function Card({ nombre, fecha, url, index }) {
  return (
    <motion.div
      ...
      className="  md:w-1/5 flex flex-col  card p-2 rounded-md"
    >
      {index === 0 && <img src={digitalHouse} alt="" />}
      {index === 1 && <img src={Oracle} alt="" />}
      {index === 2 && <img src={ArgPrograma} alt="" />}
      {index === 3 && <img src={Yopuedo} alt="" />}
      {index === 4 && <img src={Taller} alt="" />}
      {index === 5 && <img src={Reactutn} alt="" />}
      {index === 6 && <img src={Egg} alt="" />}
      <ScrollAnimation>
        <motion.div
          initial={{ opacity: 0 }}
          animate={{
        ...
        >
          <p className="font-semibold text-sm mt-2">{nombre}</p>
